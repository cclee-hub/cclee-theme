...
docker-compose 使用命名卷（如 `wp_html`）时，宿主机路径和容器路径完全隔离，宿主机看不到容器内的子主题文件。

排查顺序：
1. `docker exec <container> ls /var/www/html/wp-content/themes/` 确认容器内实际文件
2. 对比宿主机文件判断是否同步

---

## 场景十一：子主题模板覆盖 post_content

子主题模板如果不包含 `wp:post-content`，页面编辑器修改的内容不会渲染。

排查"改了没效果"：
1. 确认模板是否用 `wp:post-content`
2. 确认改的是模板文件还是页面内容
...
FSE 自定义模板不会自动分配给对应 WP 页面。

```bash
# 手动分配模板
wp post meta update <page_id> _wp_page_template <template_slug> --allow-root
```
...
## 场景十三：Site Editor 残留清理

Site Editor 保存的 wp_global_styles 如果 JSON 损坏，会导致所有页面 WP_Theme_JSON_Resolver 解析错误。
...
开发期禁止 Site Editor 保存（见 CLAUDE.md）。

---

## 场景十四：WooCommerce 示例数据图片错位

示例数据导入会产生 ID 错位：部分商品 _thumbnail_id 指向不存在的 ID 或其他商品。

排查：
1. 查 `wp_postmeta._thumbnail_id`
2. 验证目标 ID 的 post_type 是否为 attachment
3. 检查 _wp_attached_file 是否有值
